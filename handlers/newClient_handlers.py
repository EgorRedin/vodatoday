...
async def handle_district(msg: Message, state: FSMContext):
    ...
    await msg.answer("Введите номер телефона в формате - 89999999999")
...
async def handle_phone(msg: Message, state: FSMContext):
    if re.search(r'^89\d{9}$', msg.text.strip()):
        ...
        await msg.answer("Я вас не понимаю, введите номер телефона в формате - 89999999999")


@router.message(NewClient.volume)
async def handle_volume(msg: Message, state: FSMContext):
    if re.search(r'\d', msg.text.strip()):
        await state.update_data(volume=int(msg.text.strip()))
...
async def handle_empty(msg: Message, state: FSMContext):
    if msg.text.lower().strip() not in ["да", "нет"]:
        await msg.answer("Я вас не понимаю, у вас есть пустая тара?", reply_markup=kb_builder(["Да", "Нет"], [2]))
    else:
        await state.update_data(is_not_empty=(msg.text.lower().strip() == "да"))
...
async def handle_time(msg: Message, state: FSMContext):
    if msg.text.lower().strip() in ["9-12", "12-18"]:
